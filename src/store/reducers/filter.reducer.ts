...
export const filterReducer = (state = defaultState, action: filteredActionType) => {
    switch (action.type){
        ...
        case FILTER_CARDS:
            return  {...state, filtredCards: state?.data?.data.filter((hotel:hotelType) => (Number(hotel?.result_object?.rating) >= Number(state.rating)))}
        case RESET_FILTERS:  
             return {...state, filtredCards: state.data.data}
        default:
            return state;
    }
}